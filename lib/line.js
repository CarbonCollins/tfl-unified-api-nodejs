...
const LineMixin = (SuperClass) => { 
  ...
  const Line = class extends SuperClass {
    ...
    getRoutesById(ids, options = {}) {
      return this.sendRequest((options.direction && options.direction !== '') ? `/Line/${this.serialiseArray(ids)}/Route/Sequence/${options.direction}` : `/Line/${this.serialiseArray(ids)}/Route`, options, 'GET');
    }
