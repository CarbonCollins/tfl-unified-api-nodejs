...
const LineMixin = (SuperClass) => { 
  ...
  const Line = class extends SuperClass {
    ...
    getRoutesById(ids, options = {}) {
      return this.sendRequest((options.direction) ? `/Line/${this.serialiseArray(ids)}/Route/Sequence/${options.direction}` : `/Line/${this.serialiseArray(ids)}/Route`, options, 'GET');
    }
     ...
     * @param {String[]|String} ids An array or a single string of line ids e.g. circle, northern, ect
     * @param {Object} [options] optional parameters to be added to the request
    ...
    getLineStatusById(ids, options = {}) {
      return this.sendRequest((options.dateRange) ? `/Line/${this.serialiseArray(ids)}/Status/${this.serialiseDate(options.dateRange.startDate)}/to/${this.serialiseDate(options.dateRange.endDate)}` : `/Line/${this.serialiseArray(ids)}/Status` , options, 'GET');
    }
    
    /**
     * @method module:TfLUnified/api.Line~getLineStatusByMode
     * @description Gets line status information using a lines mode [GET]
     * @param {String[]|String} modes An array or a single string of line modes e.g. tube, bus, ect
     * @param {Object} [options] optional parameters to be added to the request
     * @see {@link https://api.tfl.gov.uk/swagger/ui/index.html?url=/swagger/docs/v1#!/Line/Line_StatusByMode}
     * @see {module:TfLUnified/api.Line~listValidModes}
     * @returns {Object[]} an array of specified line information
     */
    getLineStatusByMode(modes, options = {}) {
      return this.sendRequest(`/Line/Mode/${this.serialiseArray(modes)}/Status`, options, 'GET');
    }
  };
  return Line;
}
