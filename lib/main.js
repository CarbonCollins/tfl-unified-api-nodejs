...
class TfLUnified {
  ...
  /**
   * @method module:TfLUnified/api.TfLUnified~joinParams
   * @description joins an array of stirngs into a comma seperated string before sending a request
   * @param stringArray an array of strings to be concatenated and sent
   * @param uri the uri to send the final request to
   * @param method the method in which to send the request with e.g. GET, POST, ect
   * @private
   */
  joinParams(stringArray, uri, method) {
    const arrayString = (stringArray && Array.isArray(stringArray)) ? stringArray.join(',') : null;
    return (arrayString !== null)
      ? this.sendRequest(`${uri}/${arrayString}`, {}, method)
      : Promise.reject(new Error('API call requires an array of strings to be supplied'));
  }
}
